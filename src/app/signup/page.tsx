...
function convertGoogleDriveUrl(url: string): string {
  ...
  // This regex handles various Google Drive URL formats to find the file ID
  const match = url.match(/id=([^&]+)/) || url.match(/\/d\/([^/]+)/);
  if (match && match[1]) {
    const fileId = match[1];
    // Use the direct content host for Google user content
    return `https://lh3.googleusercontent.com/d/${fileId}`; 
  }
  return url; // Return original if ID extraction fails
}
...
export default function SignUpPage() {
  ...
  const handleSignUp = async (e: FormEvent) => {
    ...
    try {
      ...
      // Use the converter function to fix the photo URL before saving
      const correctedPhotoUrl = convertGoogleDriveUrl(formData.photoUrl);

      // Create user document in Firestore with a pending status
      await setDoc(doc(firestore, 'users', user.uid), {
        name: formData.name,
        saeId: null, // Will be generated by admin upon approval
        ...
        photoUrl: correctedPhotoUrl || `https://i.pravatar.cc/150?u=${user.uid}`, // Use corrected URL or fallback
        team: formData.team,
      ...
      <div className="w-full max-w-2xl p-8 space-y-8 bg-white rounded-lg shadow-md">
        ...
        {message ? (
          ...
          <form className="mt-8 grid grid-cols-1 md:grid-cols-2 gap-6" onSubmit={handleSignUp}>
            ...
            <input name="photoUrl" type="url" placeholder="Public Photo URL (e.g., from Google Drive)" onChange={handleChange} className="input-style md:col-span-2" />
